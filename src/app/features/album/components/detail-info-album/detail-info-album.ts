...
import { AlbumResponse } from '../../../../core/models/album.model';
...
@Component({
  ...
  selector: 'app-detail-info-album',
  styleUrl: './detail-info-album.css',
  templateUrl: './detail-info-album.html',
})
export class DetailInfoAlbum {
  private readonly _colorThief = inject(ColorThiefService);
  private readonly _imageHelper = inject(ImageHelperService);

  public $album = input<AlbumResponse>();
  public $totalSongs = input<number>();
  public $totalDuration = input<number>();
  ...
  public thumbnailUrl = computed(() => {
    ...
    const thumbs = this.$album()?.thumbnails;
  ...
  public onImageLoad(img: HTMLImageElement): void {
    ...
    try {
      ...
      console.warn('ColorThief: No se pudo analizar la paleta de la imagen del álbum:', err);
    }
  }
  ...
  public songCountLabel = computed(() => {
    const count = this.$totalSongs() ?? this.$album()?.songs?.length ?? 0;
    if (count === 1) return '1 canción';
    return `${count} canciones`;
  });

  public totalDurationLabel = computed(() => {
  ...
  public yearLabel = computed(() => {
    const year = this.$album()?.year;
